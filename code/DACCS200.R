...
AbSetUp <- function(
  ...
  KnownWellDensity=0.44,# WELL DENSITY - wells / km2 Known abandoned well density
  wellUnderEst=1, # WELL DENSITY - Well under-estimation factor
  UnPlugWells=00, # WELL STATUS  - Proportion of unplugged wells
  DegradWells=exp(-2.17), # PLUGGED WELL CONDITIONS - Proportion of Degraded wells
  ...
  BlowoutWellYear=0.00002, # BLOWOUTS  - Long-term blowout rate - events per well year over 10 kyr
...
SSCBase <- function(CO2target = 1.2e10,  # total injection target, tonnes
                    ...
                    ActiveWellFreq = exp(-2.17),    # Slow leak frequency, proportion active wells leaking (5)
                    ...
                    MajorBlowFreq = 1.48e-4, # Major blowout frequency (t / event / year)
                    
                    CO2MajorBlowout = exp(exp(2.57)),  # Major blowout mass (t)  (10)
                    
                    ##################  Abandonned well LEAKAGE
                    
                    KnownWellDensity= 0.44,#### well density (wells / km2)
                    
                    wellUnderEst=1, #### well underestimation factor
                    
                    UnPlugWells=0, #### proportion unplugged wells
                    
                    DegradWells=exp(-2.17), #### proportion degraded wells
...
SSCMC <- function(n=5){
  ...
  for (i in 1:n) {
    ...
    newLine <- SSCBase(CO2target = 1.2e10,  # total injection target, t, FIXED
                       ...
                       ActiveWellFreq = rlnorm(1,-2.17,0.6),    # Slow leak frequency, proportion active wells leaking (5)
                       ...
                       MajorBlowFreq = rnorm(1,1.48e-4,3.33e-5), ##### value = 4.96e-4 +/- 10%, Major blowout frequency (t / event / year)
                       
                       CO2MajorBlowout = exp(rlnorm(1,2.57,0.045)),  # NOTE: in min max this is +/- 0.045. Originally it was 0.011. Major blowout mass (t)  (10) exp(6.6)=750 t as requested 
                       
                       ##################  Abandoned well LEAKAGE
                       
                       KnownWellDensity=runif(1,0.4,0.48), #####0.44 +/- 10%, well density (wells / km2)
                       
                       wellUnderEst=1, #### well underestimation factor
                       
                       UnPlugWells=0, #### proportion unplugged wells
                       
                       DegradWells=rlnorm(1,-2.17,0.6), # proportion degraded wells
...
MinMaxSA <- as.data.frame(t(as.matrix(data.frame(
  ...
  Min_ActiveWellFreq = Basic(ActiveWellFreq=exp(-2.17-(SDN*0.6))), # SDN Standard Deviations
  Max_ActiveWellFreq = Basic(ActiveWellFreq=exp(-2.17+(SDN*0.6))), # 
  ...
  Min_MajorBlowFreq = Basic(MajorBlowFreq=1.48e-4-(SDN*3.33e-5)), # SDN Standard Deviations
  Max_MajorBlowFreq = Basic(MajorBlowFreq=1.48e-4+(SDN*3.33e-5)), #
  Min_CO2MajorBlowout = Basic(CO2MajorBlowout=exp(exp(2.57-(SDN*0.045)))), # (originally 2.257) SDN Standard Deviations
  Max_CO2MajorBlowout = Basic(CO2MajorBlowout=exp(exp(2.57+(SDN*0.045)))), # (originally 2.257) 
  Min_KnownWellDensity = Basic(KnownWellDensity=0) ,
  Max_KnownWellDensity = Basic(KnownWellDensity=5) ,
  Min_wellUnderEst = Basic(wellUnderEst=1), # Minimum value
  Max_wellUnderEst = Basic(wellUnderEst=1.1), # Maximum value
  Min_DegradWells = Basic(DegradWells=exp(-2.17-(SDN*0.6))), # SDN Standard Deviations
  Max_DegradWells = Basic(DegradWells=exp(-2.17+(SDN*0.6))), #
...
PlotLossHistogram <- function(i = 17, temp, time_points = c(1, 3, 10, 30, 100, 200, 500, seq(1000, 10000, 1000))) {
...
ggsave(
  filename = "DACCS_CaR_density_200yrs.png",
  plot = pp,
...
ggsave(
  filename = "DACCS_CaR_density_1000yrs.png",
  plot = pp,
  path = "C:/Users/bdg203/Dropbox/Exeter/Offsets/RevisitingRemovals/Nature Paper/DACCS_Figures",
  width = 8,
  height = 6,
